...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame,
                          min_missing_share: float = 10.0) -> Dict[str, Any]:
    """
    ...
    for column in summary.columns:
        if column.is_numeric:
            null_values += column.missing_share
    
    flags["has_many_zero_values"] = null_values > min_missing_share #Эвристика проверки доли нулевых значений в колонках, использование нового параметра CLI-команды (min_missing_share)
    
    #Новая эвристика - проверка датасета на наличие дубликатов в user_id
    unique_ids = False
    for column in summary.columns:
        if column.name == "user_id":
            unique_ids = column.unique == column.missing + column.non_null
            break
    flags["has_suspicious_id_duplicates"] = unique_ids #Включение нового флага в возвращаемый словарь
