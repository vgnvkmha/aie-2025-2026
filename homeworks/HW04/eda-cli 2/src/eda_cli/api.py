...
# ---------- Новый эндпоинт, завязанный на HW03 /quality-flags-from-csv ----------


@app.post("/quality-flags-from-csv", response_model = QualityFlagsResponse, tags=["quality_flags"])
def quality_flags(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    summary = summarize_dataset(df) #Вызов метода summarize_dataset
    missing_df = missing_table(df) #Вызов метода missing_table
    flags_all = compute_quality_flags(summary, missing_df) #Вызов метода compute_quality_flags
