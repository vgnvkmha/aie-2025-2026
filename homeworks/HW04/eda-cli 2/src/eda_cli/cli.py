...
def report(
    ...
    title: str =  typer.Option("EDA отчёт", help="Заголовок отчета"), #Новый дополнительный параметр
    min_missing_share: float =  typer.Option(10.0, help="Порог доли пропусков, выше которого колонка считается проблемной и попадает в отдельный список в отчёте."), #Новый дополнительный параметр
) -> None:
    """
